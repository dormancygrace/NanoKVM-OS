...
python3 "$repo/firmware/release/beta13/prepare-rootfs.py" \
  --repo "$repo" --output "$out" \
  --server "$base/universal-server-v1/NanoKVM-Server" \
  --web "$base/releases/beta11-seq27/web" \
  --capture "$base/ironkvm-c01-debug-20260914/capture/libkvm.so" \
  --board-stage "$old/board-stage" --boot "$base/universal-boot-v2"
