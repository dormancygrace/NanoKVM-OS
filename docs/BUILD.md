...
When staging a fresh OS image, pass `--version 1.0.0-beta.1 --update-sequence 3` to `stage-enhanced-app.py` for the current application. Use the actual signed release sequence for future versions. Beta image assembly now requires this metadata, the independent updater/recovery scripts and all four monitor EDID profiles.
