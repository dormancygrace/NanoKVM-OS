...
int get_edid_from_file(const char *filename, uint8_t *edid_data, uint16_t *edid_size) {
    ...
    *edid_size = fread(edid_data, 1, EDID_BUFFER_SIZE, file);
    ...
    if (failed || extra != EOF) {
        fprintf(stderr, "EDID file read failed or exceeds %d bytes\n", EDID_BUFFER_SIZE);
        return -1;
    }
    return 0;
}
...
static int wait_reset_interval(void)
{
    struct timespec left = { .tv_sec = 0, .tv_nsec = 100000000 };
...
static int reset_write(int fd, const char *value)
{
...
static int enhanced_image(void)
{
    char marker[256];
    FILE *file = fopen("/etc/nanokvm-buildroot", "r");
    if (!file) return errno == ENOENT ? 0 : -1;
    if (!fgets(marker, sizeof(marker), file)) {
        fclose(file); errno = EIO; return -1;
    }
    if (fclose(file) != 0) return -1;
    if (strstr(marker, "flavour=enhanced")) return 1;
    if (strstr(marker, "flavour=compatibility")) return 0;
    errno = EINVAL;
    return -1;
}

static int maintenance_lock(void)
{
    int fd = open("/run/nanokvm-edid.lock", O_CREAT | O_RDWR | O_CLOEXEC, 0600);
    if (fd < 0) return -1;
    if (flock(fd, LOCK_EX | LOCK_NB) < 0) {
        int saved = errno; close(fd); errno = saved; return -1;
    }
    return fd; /* Held until process exit, including both resets and programming. */
}

int NanoKVM_PCIe_HDMI_Reset(void)
{
    /* An Enhanced image must use the owning driver; no global-number fallback. */
    int enhanced = enhanced_image();
    if (enhanced < 0) return -1;
    const char *path = enhanced ? "/sys/bus/platform/devices/hdmi-reset/pulse"
                                : "/sys/class/gpio/gpio451/value";
    int fd = open(path, O_WRONLY | O_CLOEXEC);
    if (fd < 0) return -1;
    int ret = reset_write(fd, enhanced ? "1" : "0");
    int saved = ret ? errno : 0;
    if (!enhanced) {
        if (!ret && wait_reset_interval() < 0) { ret = -1; saved = errno; }
        /* Release even if assertion/wait failed. */
        if (reset_write(fd, "1") < 0) { ret = -1; saved = errno; }
        else if (wait_reset_interval() < 0) { ret = -1; saved = errno; }
    }
    if (close(fd) < 0 && !ret) { ret = -1; saved = errno; }
    if (ret) errno = saved;
    return ret;
}
...
{
    if (!len || len > EDID_BUFFER_SIZE) { errno = EINVAL; return -1; }
...
int lt6911_edid_config(chip_version_t chip_version, uint8_t *edid_data, uint16_t edid_size)
{
    ...
    // Avoid wearing the receiver's flash when the requested profile is already
    // installed. A failed read is not permission to overwrite unknown state.
    uint8_t current_edid[EDID_BUFFER_SIZE] = {0};
    int read_ret = -1;
    if (chip_version == CHIP_LT6911UXC)
        read_ret = lt6911uxc_edid_read(current_edid, EDID_BUFFER_SIZE);
    else if (chip_version == CHIP_LT6911C)
        read_ret = lt6911c_edid_read(current_edid, EDID_BUFFER_SIZE);
    else if (chip_version == CHIP_LT6911D)
        read_ret = lt6911d_edid_read(current_edid, EDID_BUFFER_SIZE);
    if (read_ret != 0) {
        fprintf(stderr, "Cannot read existing EDID; refusing to program\n");
        close(client);
        return -1;
    }
    if (memcmp(current_edid, edid_data, EDID_BUFFER_SIZE) == 0) {
        puts("Requested EDID already installed; flash write skipped");
        close(client);
        return 0;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3 && strcmp(argv[1], "--check") == 0) {
        uint8_t data[EDID_BUFFER_SIZE];
        uint16_t size = 0;
        if (get_edid_from_file(argv[2], data, &size) || check_edid(data, size)) return 1;
        puts("EDID size, header and block checksums passed; no hardware access");
        return 0;
    }
    ...
    if (argc != 2) {
        fprintf(stderr, "Usage: %s EDID.bin | --reset-only | --check EDID.bin\n", argv[0]);
        return 1;
    }
    int lock_fd = maintenance_lock();
    if (lock_fd < 0) { perror("EDID maintenance lock"); return 1; }

    if (argc == 2 && strcmp(argv[1], "--reset-only") == 0) {
        if (product_version != PRODUCT_PCIE_A) {
            fprintf(stderr, "HDMI reset is available only for PCIe hardware\n");
            return 1;
        }
        if (NanoKVM_PCIe_HDMI_Reset() < 0) { perror("HDMI reset failed"); return 1; }
        puts("HDMI reset completed; EDID was not written");
        return 0;
    }
    ...
    if (product_version == PRODUCT_PCIE_A && NanoKVM_PCIe_HDMI_Reset() < 0) {
        perror("HDMI reset failed");
        return EXIT_FAILURE;
    }
