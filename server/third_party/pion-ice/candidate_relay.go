...
// RelayTransportAddress returns the resolved local TURN UDP endpoint, if known.
func (c *CandidateRelay) RelayTransportAddress() string { return c.transportAddress }
...
func (c *CandidateRelay) copy() (Candidate, error) {
	...
	if ccr, ok := cc.(*CandidateRelay); ok {
		...
		ccr.transportAddress = c.transportAddress
	}
