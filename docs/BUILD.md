...
This builds the server, the static `nkos-update` helper and a hash manifest. Preserve the source's local Pion SRTP replacement in `server/third_party/pion-srtp`. The helper must be installed in the system image as `/usr/sbin/nkos-update`, with the early `S00nkos-system-update` dispatcher, `S94nanokvm-update`, and late `S99nkos-system-confirm` health confirmation. The system foundation is generated by the Buildroot post-build hook.
