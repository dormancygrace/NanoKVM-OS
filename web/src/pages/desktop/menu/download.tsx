...
export const DownloadImage = () => {
  ...
  function getDownloadStatus(generation = pollingGeneration.current) {
    statusImage()
      ...
      .catch(() => {
        if (generation !== pollingGeneration.current) return;
        setLog(t('capture.downloadStatusFailed'));
      });
  }
      ...
      {diskEnabled !== true && (
        <div className="mb-2 flex items-center gap-2 text-amber-400">
          <span>
            {t(
              diskCheckFailed
                ? 'capture.storageCheckFailed'
                : diskEnabled === false
                  ? 'capture.storageUnavailable'
                  : 'capture.checking'
            )}
          </span>
          <Button size="small" onClick={checkDiskEnabled}>
            {t('settings.usb.reload')}
          </Button>
        </div>
      )}
      {diskEnabled !== true && isRemoteDownloading && status === 'in_progress' && (
        <Button danger loading={isCancelling} onClick={cancelDownload}>
          {t('download.cancel')}
        </Button>
      )}
      {diskEnabled === true && (
