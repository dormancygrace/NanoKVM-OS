...
const MobileMenuPanel = ({
  ...
  const push = useCallback((entry: MobileMenuNavigationEntry) => {
    ...
    current
      .slice(index === -1 ? current.length : index + 1)
      .reverse()
...
export const MenuItem = ({
  ...
  const previousCloseSignal = useRef(menuCloseSignal);
  useEffect(() => {
    if (previousCloseSignal.current !== menuCloseSignal) {
      previousCloseSignal.current = menuCloseSignal;
      close();
    }
  }, [close, menuCloseSignal]);
  useEffect(() => {
    if (disabled) close();
  }, [close, disabled]);
  useEffect(() => {
