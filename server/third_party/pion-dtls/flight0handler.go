...
func flight0Parse(
	...
	// A DTLS server selects from its configured preference order. Client-side
	// selection and validation are unchanged.
	if state.cipherSuite, ok = findMatchingCipherSuite(cfg.localCipherSuites, cipherSuites); !ok {
