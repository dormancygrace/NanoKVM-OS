...
parser.add_argument("--localversion", default="-nanokvm-os")
args = parser.parse_args()
config={}
for line in Path(args.config).read_text().splitlines():
    if line.startswith('CONFIG_') and '=' in line:
        key,value=line.split('=',1);config[key[7:]]=value
required_builtin='''ARCH_SOPHGO ERRATA_THEAD ERRATA_THEAD_MAE ERRATA_THEAD_CMO ERRATA_THEAD_PMU ERRATA_THEAD_GHOSTWRITE RISCV_ISA_XTHEADVECTOR MODULES DEVTMPFS DEVTMPFS_MOUNT BLK_DEV_INITRD ION ION_CARVEOUT_HEAP ION_CVITEK NANOKVM_EFUSE NANOKVM_HDMI_RESET PINCTRL_SOPHGO_SG2002 CLK_SOPHGO_CV1800 GPIO_DWAPB GPIO_CDEV I2C_DESIGNWARE_PLATFORM I2C_CHARDEV I2C_GPIO SERIAL_8250_DW SERIAL_8250_CONSOLE SERIAL_EARLYCON_RISCV_SBI RISCV_SBI_V01 SOPHGO_CV1800_RTCSYS STMMAC_ETH DWMAC_SOPHGO MDIO_BUS_MUX MDIO_BUS_MUX_MMIOREG NANOKVM_CVITEK_PHY MMC MMC_SDHCI_OF_DWCMSHC PWRSEQ_SIMPLE USB_DWC2 USB_DWC2_DUAL_ROLE PHY_SOPHGO_CV1800_USB2 USB_GADGET USB_CONFIGFS USB_CONFIGFS_SERIAL USB_CONFIGFS_ACM USB_CONFIGFS_NCM USB_CONFIGFS_ECM USB_CONFIGFS_MASS_STORAGE USB_CONFIGFS_F_HID INPUT_EVDEV KEYBOARD_GPIO LEDS_GPIO CONFIGFS_FS EXT4_FS VFAT_FS EXFAT_FS TMPFS TUN'''.split()
required_builtin += 'NF_CONNTRACK_MARK IP_MULTIPLE_TABLES IPV6_MULTIPLE_TABLES CMA DMA_CMA ZRAM_MULTI_COMP ZRAM_BACKEND_LZ4 ZRAM_BACKEND_ZSTD ZRAM_TRACK_ENTRY_ACTIME LRU_GEN LRU_GEN_ENABLED FLATMEM SECCOMP SECCOMP_FILTER STACKPROTECTOR_STRONG STRICT_MODULE_RWX'.split()
required_available='WIREGUARD NFT_FIB_IPV4 NFT_FIB_IPV6 ZRAM NF_TABLES NFT_CT NFT_COMPAT NF_NAT NFT_NAT NFT_MASQ NFT_REDIR NETFILTER_XT_MATCH_STATE CFG80211 BT BT_RFCOMM BT_BNEP BT_HIDP'.split()
errors=[]
