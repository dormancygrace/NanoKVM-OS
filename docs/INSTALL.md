...
**NanoKVM OS v1.0.0 beta-1 — full image, 2026-09-10 build (application sequence 3).** Includes Linux 7.2.4, current native/video components, QHD, CryptoDMA and OpenVPN 2.7.7/DCO. HTTPS is enabled on first installation and H.265 Direct is the default for a fresh browser.
...
This first release contains **`NanoKVM-OS-1.0.0-beta.1.img.zip`**, the full SD image. Use it for original NanoKVM firmware, a blank card or an incompatible OS base. A separate `.nkos` package is not distributed for beta-1; application updates are intended for later releases on an already installed compatible NanoKVM OS. Original NanoKVM application archives are not NanoKVM OS updates.
...
1. Verify the complete SHA256 digest against the release's `SHA256SUMS`. Extract the `.img.zip` archive. On Windows, right-click it and choose **Extract All…**; on Linux/macOS, use an archive manager or `unzip NanoKVM-OS-1.0.0-beta.1.img.zip`. Verify the unpacked `.img` digest too.
...
The raw image is 1,627,390,464 bytes; the compressed ZIP is about 56.3 MiB. The first-boot script creates the remaining data partition. Its behavior on a fresh card, this exact image's hardware USB recovery, and Cube-specific operation still need physical qualification.
...
Use **Settings → Updates** for future compatible application/web releases. The image initializes installed release sequence 3. Future signed packages need a higher sequence and the matching native-library fingerprint. Kernel/rootfs or native-library changes require another complete image.
