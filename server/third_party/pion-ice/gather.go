			...
			for _, localBindAddr := range bindAddrs {
				...
				go func(url stun.URI, network string, urlProto stun.ProtoType, localBindAddr string) {
					...
					case urlProto == stun.ProtoTypeUDP && url.Scheme == stun.SchemeTypeTURN:
						// Resolve once for the opt-in path observer; the TURN client then
						// receives this same numeric endpoint, avoiding a second DNS choice.
						if _, observing := a.net.(PathMTUObserver); observing {
							endpoint, resolveErr := a.net.ResolveUDPAddr(network, turnServerAddr)
							if resolveErr != nil {
								a.log.Warnf("Failed to resolve TURN route: %v", resolveErr)
								return
							}
							transportAddress = endpoint.String()
							turnServerAddr = transportAddress
						}
...
func (a *Agent) createRelayCandidate(ctx context.Context, ep relayEndpoint, ip net.IP, onClose func() error) error {
	...
	candidate.transportAddress = ep.transportAddress
