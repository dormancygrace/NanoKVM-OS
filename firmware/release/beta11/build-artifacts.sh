#!/bin/bash
# Current beta-11 distribution: full SD image only, no .nkos package.
set -euo pipefail
...
python3 "$repo/scripts/assemble-enhanced-test-image.py" \
  ...
  --epoch "$epoch" --version 1.0.0-beta.11
...
names=release_names('1.0.0-beta.11')
...
manifest={'version':'1.0.0-beta.11','sequence':27,'kernel':'7.2.5-nanokvm-os-r3',
          ...
          'prior_acceptance':'Sequence 26 full package passed beta10 installation; OLED, DNS and USB Serial web changes separately device-tested. Combined sequence 27 image not device-flashed.'}
(dest/'build-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
print(json.dumps(files,indent=2))
PY
echo BETA11_IMAGE_ONLY_READY
