...
// CPU view of the actual NV21 plane layout; data remains a VI lease.
typedef struct {
    uint8_t *data;
    uint32_t length;
    uint32_t width, height;
    uint32_t y_stride, vu_stride, vu_offset;
} mmf_nv21_view_t;
...
// Capture owner must serialize calls; busy encoder storage is retained.
