#!/usr/bin/env python3
"""Apply the beta-12 release delta to a mounted pristine beta-10 image."""
import argparse
import hashlib
import json
import os
...
for name in ('repo', 'output', 'server', 'web', 'capture', 'board-stage'):
...
assert sha(a.server) == '1fa6cf7ce6e8aa13c0bf4b77595101627a914903b37322cad8e8dfba86e30cfa'
assert sha(a.web / 'index.html') == '1c270344e3f4db20c7f6841b1506284e6d4fbb77b787deb16361a08f099d5408'
assert sha(a.capture) == '3fecf95c740beaa5396046d7b8217969b857966959b0ba5998670c6d4fb498ee'
...
for name in ('S30eth', 'S30wifi', 'S34mssclamp', 'S49persistent-cron'):
    for prefix in ('etc/init.d', 'kvmapp/system/init.d'):
...
version, sequence = '1.0.0-beta.12', 28
...
assert (root / 'usr/lib/modules/7.2.5-nanokvm-os-r3').is_dir()
...
manifest = {
    ...
    'inputs': {'base': 'pristine beta-10 seq23 image',
               'hardware_detection_commit': '6cea30380a8f9d45198d426728e8dcd5a49ce460',
               'server_commit': '8fc52ce1fa9e3e8f54dbf03c99f015e2e5e0b84c',
               'web_commit': '02528afcc2fe0ec192e5b7b45504c765491ec5ae',
               'capture_commit': '0eee8b7e24fdff33518a190c0466ff66aff10dcf'},
...
(out / 'rootfs-delta.json').write_text(json.dumps({
    'version': version, 'sequence': sequence, 'kernel': '7.2.5-nanokvm-os-r3',
    ...
    'system_base': base_before.strip(), 'application_manifest_sha256': sha(manifest_path),
    'ssh_default': 'disabled', 'python_in_base': False,
}, indent=2) + '\n')
print('BETA12_ROOTFS_STAGED')
