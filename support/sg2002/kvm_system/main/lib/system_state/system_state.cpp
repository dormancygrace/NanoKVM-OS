...
int read_integer_file(const char* path, int fallback)
{
...
void publish_wifi_state(uint8_t state)
{
	...
	FILE* fp = fopen("/run/nanokvm/wifi_state", "w");
	if (fp == NULL) {
		return;
	}
	...
	switch (ip_type){
        ...
        case RNDIS_IP: {
            ...
            uint8_t *destination = ip_type == ETH_IP ? kvm_sys_state.eth_addr :
                ...
                ip_type == Tailscale_IP ? kvm_sys_state.tail_addr : kvm_sys_state.rndis_addr;
            const auto addresses = ip_address();
            const auto found = addresses.find(interface);
            memset(destination, 0, 16);
            if (found == addresses.end() || found->second.empty() || found->second.size() >= 16)
                return 0;
            memcpy(destination, found->second.data(), found->second.size());
            return 1;
        }
...
void kvm_update_stream_fps(void)
{
	...
	// FPS
	fp = fopen("/run/nanokvm/now_fps", "r");
    fseek(fp, 0, SEEK_END);
    file_size = ftell(fp); 
    fseek(fp, 0, SEEK_SET);
    fread(RW_Data, sizeof(char), file_size, fp);
...
{
	kvm_sys_state.hdmi_width = read_integer_file("/run/nanokvm/width", 0);
	kvm_sys_state.hdmi_height = read_integer_file("/run/nanokvm/height", 0);
}
