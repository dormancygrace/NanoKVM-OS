...
func TestResolutionCeilingPreservesRequestedFPS(t *testing.T) {
	for _, keys := range [][]string{{"fps", "resolution"}, {"resolution", "fps"}} {
		...
		for _, key := range keys {
			...
			setScreenValue(screen, key, value)
		}
		if screen.Width != 2560 || screen.Height != 1440 || screen.FPS != 60 {
			t.Fatalf("order %v produced %+v", keys, screen)
		}
		setScreenValue(screen, "fps", 24)
		if screen.FPS != 24 {
			t.Fatal("QHD must preserve a lower FPS request")
		}
		setScreenValue(screen, "resolution", 1080)
		setScreenValue(screen, "fps", 60)
		if screen.FPS != 60 {
			t.Fatal("FHD must still allow 60 FPS")
		}
	}
}

func TestLoadQHDPersistedAt60FPS(t *testing.T) {
	files := map[string][]byte{
		"/kvmapp/kvm/fps": []byte("60"),
		"/kvmapp/kvm/res": []byte("1440"),
	}
