...
func (h *Hid) OpenNoLock() error {
	...
	for _, device := range h.devices() {
		if h.deviceDisabledNoLock(device.path) {
			continue
		}
...
func (h *Hid) writeHID(device hidDevice, data []byte) error {
	...
	// Input to an explicitly disabled function is a no-op. Do not reopen a
	// missing device or hold the input queue in repeated timeout/retry loops.
	if h.deviceDisabledNoLock(device.path) {
		return nil
	}
...
func disabledHIDFunctions(bootDir string) (keyboard, relative, absolute bool) {
	exists := func(name string) bool {
		_, err := os.Stat(filepath.Join(bootDir, name))
		return err == nil
	}
	all := exists("disable_hid")
	return all || exists("usb.disable_keyboard"), all || exists("usb.disable_relative"), all || exists("usb.disable_absolute")
}

// Caller holds the matching input lock (both locks while refreshing settings).
func (h *Hid) deviceDisabledNoLock(path string) bool {
	switch path {
	case HID0:
		return h.keyboardDisabled
	case HID1:
		return h.relativeDisabled
	case HID2:
		return h.absoluteDisabled
	default:
		return false
	}
}
