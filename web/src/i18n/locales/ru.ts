const ru = {
  translation: {
    videoSettings: {
      ...
      monitorUnavailable:
        'Смена профиля пока доступна на NanoKVM PCIe с LT6911UXC. Входной сигнал определяется автоматически.',
      stream: 'Трансляция',
      streamHint:
        'Разрешение, FPS и битрейт действуют для всех зрителей. Разрешение трансляции не меняет рабочий стол компьютера. Способ передачи и кодек выбираются для этого браузера.',
    ...
    settings: {
      updates: {
        title: 'Обновления',
        description:
          'Обновление приложения и веб-интерфейса NanoKVM OS. Во время установки видео и управление ненадолго отключатся.',
        ...
        file: 'Подписанный пакет приложения (.nkos)',
        verify: 'Загрузить и проверить',
        install: 'Установить',
        compatibility:
          'Принимаются только подписанные пакеты NanoKVM OS, совместимые с этой системой. Архивы оригинальной NanoKVM отклоняются. Полные образы устанавливаются через процедуру прошивки с Boot.',
        requestFailed:
          'Запрос обновления не выполнен. Проверьте связь с устройством и повторите попытку.',
        tooLarge: 'Размер пакета превышает 96 МиБ.',
        reconnecting: 'Ожидание подключения приложения…',
