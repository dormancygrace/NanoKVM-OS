# 🚀 NanoKVM OS v1.0.0 beta-8 — Massive Dependency Refresh, APK & Device Improvements

A massive dependency refresh across the OS, backend, browser interface and build tools — now combined with an APK-managed application layer, improved Wi-Fi settings and a cleaner USB Serial Console. Beta-8 carries forward the refreshed foundation from beta-5 and brings the latest device, video and update improvements together in a complete SD image.
...
- 🛠️ **Build compatibility fixes:** refreshed cross-compilation patches, dependency locks and source/license hashes so the updated components build together. Beta-8 also checks for malformed ELF interpreters, removes stale optional-package files and preserves existing SSH preferences during full-system updates.
...
**For a fresh installation, use the complete beta-8 SD image.** It includes the system, matching kernel/modules, firmware and the APK application layer.

**Already using a compatible full-system updater? Install the signed `.nkos` package through Settings → Updates.** Compatible cumulative packages do not require installing intermediate releases. Each contains the complete system filesystem and matching kernel/modules, including system libraries, firmware, utilities and boot scripts. Supported settings, including the SSH preference and video settings, are preserved; installed APK applications are migrated through the package manager, and files on the data partition are retained. Installation restarts the device into a RAM installer and then back into the updated OS.

If the installed updater does not support the package format, or a release requires an incompatible bootloader or partition layout, use the full SD image. There is no A/B rollback; interrupted installation can require reflashing.
