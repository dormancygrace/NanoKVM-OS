...
archive=out/'opus-1.6.1.tar.gz';expected='6ffcb593207be92584df15b32466ed64bbec99109f007c82205f0194572411a1'
if not archive.exists():urllib.request.urlretrieve('https://downloads.xiph.org/releases/opus/opus-1.6.1.tar.gz',archive)
...
manifest={str(f.relative_to(out)) if f.is_relative_to(out) else str(f.relative_to(repo)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [helper,*module_artifacts]}
