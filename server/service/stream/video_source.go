...
// IsKeyframe reports the native encoder classification shared by the transports.
func (f VideoFrame) IsKeyframe() bool { return f.Result == keyFrameResult }
...
func (s *VideoSource) subscribe(config EncoderConfig) (*VideoSubscription, error) {
	for {
		...
		subscription := &VideoSubscription{
			source:  s,
			session: session,
			frames:  make(chan VideoFrame, 24),
			done:    make(chan struct{}),
			// The shared/native encoder may already be in the middle of a GOP.
...
func (s *VideoSubscription) send(frame VideoFrame) bool {
	...
	// Native errors remain observable while a new decoder waits for an IDR.
	if s.waitingForKeyframe && frame.Result >= 0 {
